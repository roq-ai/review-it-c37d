...
import { getMediaGenreById } from 'apiSdk/media-genres';
import { MediaGenreInterface } from 'interfaces/media-genre';

function MediaGenreViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('media_genre', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/media-genres/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('genre', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Genre"
                    text={
                      <Link as={NextLink} href={`/genres/view/${data?.genre?.id}`}>
                        {data?.genre?.name}
                      </Link>
                    }
                  />
