...
import { getMediaGenres } from 'apiSdk/media-genres';
import { MediaGenreInterface } from 'interfaces/media-genre';

import { MediaGenreListPage as ShowCaseList } from 'pages/media-genres';

function DashboardPage() {
  const { session } = useSession();

  const genreStat = useSWR<PaginatedInterface<GenreInterface>>(
    () => `/genres`,
    () => getGenres({ limit: 1, offset: 0 }),
  );
  ...
  const mediaGenreStat = useSWR<PaginatedInterface<MediaGenreInterface>>(
    () => `/media-genres`,
    () => getMediaGenres({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Genres',
      result: genreStat?.data?.totalCount,
      isLoading: genreStat.isLoading,
    ...
    {
      title: 'Media Genres',
      result: mediaGenreStat?.data?.totalCount,
      isLoading: mediaGenreStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`media-genres`} passHref legacyBehavior>
                  <Button
